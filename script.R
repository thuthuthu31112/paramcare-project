...
analyze_country <- function(file_path, country_name) {
  ...
  # Create PCA vector table
  pca_vector <- data.frame(
    Variable = c("Blood Pressure", "Cholesterol", "Diabetes"),
    PCA_Weight = round(weights[, 1], 4),
    Contribution = round(pca$rotation[,1]^2 / sum(pca$rotation[,1]^2) * 100, 2)
  )
  ...
  # Save PCA weights to CSV
  pca_file <- paste0("output/pca_weights_", country_name, ".csv")
  write.csv(pca_vector, pca_file, row.names = FALSE)
  
  return(list(
    country = country_name,
    correlation_matrix = corr_matrix,
    correlation_table = corr_table,
    pca_weights = pca_vector,
    pca_object = pca,
...
cat("-----------\n")
cat("Palominia:\n")
print(results_palominia$pca_weights)
cat("\nAmbernia:\n")
print(results_ambernia$pca_weights)
